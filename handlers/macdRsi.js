...
module.exports = (bot) => {
                    ...
                    if (lastMacd_5.histogram > 0 && lastMacd_15.histogram > 0 && lastMacd_1h.histogram > 0 && lastMacd_1d.histogram > 0 && lastRsi_1d < 40) {
                        ...
                        // Отправка сообщения в группу
                         await bot.api.sendMessage(CHAt_ID, ` 
                            <b>${symbol}</b>
                            На основе анализа индикаторов MACD и RSI, найден оптимальный момент для входа в сделку с минимальной вероятностью убытков.<br>
                            Текущая ситуация:<br>
                            <b>MACD:</b> На графиках таймфрейма 1D, 1H, 15m, 5m наблюдается пересечение MACD линии и сигнальной линии вверх, что указывает на возможный восходящий тренд.<br>

                            <b>RSI:</b>  Индекс относительной силы таймфрейма 1D находится в зоне ${lastRsi_1d}, что говорит о том, что актив может быть недооценен и есть потенциал для роста.<br>
                           <b>Рекомендация:</b> <br>
                            С учетом сигналов от MACD и RSI, сейчас может быть хороший момент для открытия <b>длинной позиции</b>. Однако не забудь учесть уровень стоп-лосса и тейк-профита, чтобы минимизировать риски. <br>
                           `,{
  parse_mode: "HTML",reply_markup: message_kb});
                        ...
                        await bot.api.sendMessage(CHAt_ID,`
                            <b>${symbol}</b>
                            На основе анализа индикаторов MACD и RSI, найден скорый разворот тренда.<br>
                            Текущая ситуация:<br>
                            <b>MACD:</b> На графиках таймфрейма 1D, 1H, 15m, наблюдается пересечение MACD линии и сигнальной линии вверх, таймфрейм 5m еще находится в нисходящей зоне<br>

                            <b>RSI:</b>  Индекс относительной силы таймфрейма 1D находится в зоне ${lastRsi_1d}, что говорит о том, что актив может быть недооценен и есть потенциал для роста.<br>
                           <b>Рекомендация:</b> <br>
                            С учетом сигналов от MACD и RSI, сейчас может быть хороший момент для открытия <b>короткой позиции</b>. Однако не забудь учесть уровень стоп-лосса и тейк-профита, чтобы минимизировать риски. <br>
                           `,{
  parse_mode: "HTML", reply_markup: message_kb});
                    } else {
