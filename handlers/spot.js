...
// // Функция для получения списка спотовых пар
// async function getSpotMarkets() {
//     try {
//         const markets = await exchange.fetchMarkets();
//         const spotMarkets = markets.filter(market => market.spot); // Фильтруем спотовые пары
//         logger.info(`Получено ${spotMarkets.length} спотовых пар`);
//         return spotMarkets.map(market => market.symbol); // Возвращаем только символы
//     } catch (error) {
//         logger.error(`Ошибка при получении списка спотовых пар: ${error.message}`);
//         return [];
//     }
// }
...
//             await ctx.reply('Не удалось получить список спотовых пар.');
//             return;
//         }

//         await ctx.reply(`Начинаю мониторинг спотовых пар: ${spotMarkets.join(', ')} на таймфреймах: ${timeframes.join(', ')}`);
//         logger.info(`Мониторинг запущен для спотовых пар: ${spotMarkets.join(', ')} на таймфреймах: ${timeframes.join(', ')}`);

//         // Запуск мониторинга каждые 10 секунд
//         setInterval(async () => {
//             for (const symbol of spotMarkets) {
